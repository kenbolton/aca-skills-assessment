...
export function sessionToCsv(session) {
  ...
  const rows = [['Paddler', 'Target', 'Landing', 'Category', 'Skill', 'Optional', 'Rating', 'CMS Grade', 'Feedback']];
  for (const r of session.results) {
    ...
    const cms = sk.optional ? '' : cmsGrade(r.rating);
    rows.push([p.name, p.target, landingById.get(r.paddlerId) || '', sk.category, skillLabel(sk), sk.optional ? 'yes' : '', opt ? opt.label : '', cms, r.feedback]);
  }
